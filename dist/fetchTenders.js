...
function extractCookieHeader(response, existing) {
...
async function requestWithRetry(options, maxRetries = 4, baseDelayMs = 1000) {
...
async function resolveSourceUrlPostback($, cookie, linkTarget) {
    try {
...
function classify(previous, row, hash) {
    if (!previous)
        return 'NEW_LISTING';
...
export async function fetchTenders(options) {
    ...
    const rawIds = new Set(); // dedup + raw maxItems cap, same role the old `seenIds` Set played
    const tenders = [];
    const observedThisRun = [];
    async function processRows(rows, $, cookie) {
        let added = 0;
        for (const row of rows) {
            if (rawIds.size >= maxItems)
                break;
            if (rawIds.has(row.numeroProceso))
                continue;
            ...
            if (allowedEventTypes && eventType !== 'UNCHANGED' && !allowedEventTypes.has(eventType))
                continue;
            if (dateRange && !isWithinDateRange(parseFechaApertura(row.fechaApertura), dateRange, now))
                continue;
            const sourceUrl = shouldResolveSourceUrl && row.linkTarget ? await resolveSourceUrlPostback($, cookie, row.linkTarget) : null;
            tenders.push({
    ...
    for (let pageNum = 2; pageNum <= MAX_PAGES_SAFETY_CAP && rawIds.size < maxItems; pageNum++) {
        ...
        if (pageNum % 20 === 0 || rows.length < PAGE_SIZE) {
            log.info(`Pagina ${pageNum}: ${rows.length} procesos (acumulado crudo: ${rawIds.size}, en salida: ${tenders.length})`);
        }
    }
