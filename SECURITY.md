...
- **No credential handling of any kind.** This Actor requires no third-party API key or BYOK secret and no separate `comprar.mendoza.gov.ar` credential (see the README's Cost & BYOK Disclosure section) — it walks the portal's own public advanced-search form. There is no customer secret this Actor could leak.
- **No user-supplied code execution.** Input is a fixed JSON schema (`maxItems`, `onlyNew`, `eventTypes`, `resolveSourceUrl`, `dateRange`, see `.actor/input_schema.json`) — there is no arbitrary-code or arbitrary-URL input surface.
...
- **Source-portal integrity** (a compromised or spoofed `comprar.mendoza.gov.ar` endpoint, or session-cookie mishandling) is outside this Actor's control beyond standard TLS — it captures and forwards the portal's own `ASP.NET_SessionId` and secondary session cookie strictly within the same run, and does not persist or expose them outside the Actor's own execution.
